...
export default defineApi( async ( event ) => {
  ...
  const res = await db.collection( "categories" ).insertOne( { name: parsed.data.name, slug: parsed.data.slug } )
  const category = await db.collection( "categories" ).findOne( { _id: res.insertedId } )
